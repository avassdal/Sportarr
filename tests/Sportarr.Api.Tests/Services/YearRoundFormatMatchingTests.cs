...
public class YearRoundFormatMatchingTests
{
    ...
    [Fact]
    public void YearXRoundRelease_RoundIsUsedForTheMismatchGuard()
    {
        // The round embedded in "2026x02" must feed the round-mismatch guard,
        // otherwise the year bonus alone could push a wrong-round release over
        // the threshold for a same-year event. Round 02 != event Round 3.
        var result = _svc.ValidateRelease(
            Rel("Formula.1.2026x02.China.Race.SkyF1HD.1080p"), ChineseGrandPrix(2026, round: "3"));
