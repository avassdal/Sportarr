...
public class SportsFileNameParserTests
{
    ...
    [InlineData("Formula.1.2026x02.China.Race.SkyF1UHD.4K-GROUP", 2026, 2)]
    [InlineData("Formula.1.2026x02.China.Race.SkyF1HD.1080p", 2026, 2)]
    [InlineData("Formula.1.2026x02.China.Qualifying.SkyF1UHD.4K-GROUP", 2026, 2)]
    [InlineData("Formula.1.2026x02.China.Qualifying.SkyF1HD.1080p", 2026, 2)]
    [InlineData("Formula.1.2026x02.China.Sprint.Race.SkyF1UHD.4K-GROUP", 2026, 2)]
    [InlineData("Formula.1.2026x02.China.Sprint.Qualifying.SkyF1HD.1080p", 2026, 2)]
    [InlineData("Formula.1.2026x01.Australia.Race.SkyF1HD.1080p", 2026, 1)]
    // Single-digit round and uppercase separator are handled too.
    [InlineData("Formula.1.2025x9.Canada.Race.SkyF1HD.1080p", 2025, 9)]
    [InlineData("Formula.1.2024X05.Miami.Race.SkyF1HD.1080p", 2024, 5)]
    public void Parse_YearXRoundFormat_ExtractsYearAndRound(string filename, int expectedYear, int expectedRound)
    {
        var result = _parser.Parse(filename);

        result.EventYear.Should().Be(expectedYear);
        result.RoundNumber.Should().Be(expectedRound);
    }
